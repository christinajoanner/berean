...
# Email input is rendered later in the app, inside "Save your analysis (optional)".
...
def run_simulation(p: dict, n_sims: int = 10_000, seed: int = 42) -> dict:
    ...
    # First amortizing year (or year 3 if IO is shorter), capped at last hold year
...
# Wynwood Miami boutique example
...
def clear_deal_inputs():
    ...
    # Reset model defaults that the example may have overridden (LTV, rate, etc.)
    for k, v in MODEL_DEFAULTS.items():
        st.session_state[k] = v
    st.session_state.show_results = False
...
st.markdown(f"""
<div style="font-family:'Manrope',sans-serif; font-size:0.95rem;
            color:rgba(240,240,240,0.72); line-height:1.7;
            max-width:78ch; margin-bottom:1.5rem;">
  Probabilistic underwriting for hospitality deals. Run your assumptions
  across thousands to millions of simulated futures and download a memo
  you can share with your LPs. Click
  <strong style="color:{GOLD};">Try Cartagena example</strong> or
  <strong style="color:{GOLD};">Try Wynwood (Miami) example</strong>
  below to see the output before entering your own deal.
</div>
""", unsafe_allow_html=True)
...
fa_firm, _ = st.columns([3, 5])
with fa_firm:
    st.markdown('<p class="var-label">Firm name (appears on memo)</p>', unsafe_allow_html=True)
    st.text_input("firm_name_input", key="firm_name",
    ...
    with st.form("feedback_form"):
        ...
        if submitted:
            ...
            if saved_feedback:
                ...
                st.warning("Airtable is not configured — feedback was not saved. Add credentials in Streamlit Secrets to enable tracking.")
    ...
    with st.form("save_analysis_form"):
        ...
        if save_analysis_clicked:
            ...
            deal_payload = {
                ...
                "adr_lo": float(p["adr_lo"]),
                "adr_mode": float(p["adr_mode"]),
                "adr_hi": float(p["adr_hi"]),
                "occ_lo": float(p["occ_lo"]),
                "occ_mode": float(p["occ_mode"]),
                "occ_hi": float(p["occ_hi"]),
                "cpk_lo": float(p["cpk_lo"]),
                "cpk_mode": float(p["cpk_mode"]),
                "cpk_hi": float(p["cpk_hi"]),
                "cap_lo": float(p["cap_lo"]),
                "cap_mode": float(p["cap_mode"]),
                "cap_hi": float(p["cap_hi"]),
                "ltv": float(p["ltv"]),
            ...
            if saved_deal:
                ...
                st.info("Analysis is ready to save, but Airtable is not configured yet. Add credentials in Streamlit Secrets to enable tracking.")
            else:
