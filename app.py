...
def get_benchmarks(market: str) -> dict:
    ...
    return BENCHMARKS.get(market, BENCHMARKS["Caribbean Resort"])
...
def _flag(val, lo, hi, label, fmt=""):
    if val is None:
        return
    if val > hi * 1.15:
        st.warning(f"⚠️ {label} {fmt}{val} is {int((val/hi-1)*100)}% above market range {fmt}{lo}–{fmt}{hi}. Confirm from comps.")
    elif val < lo * 0.85:
        st.warning(f"⚠️ {label} {fmt}{val} is {int((1-val/lo)*100)}% below market range {fmt}{lo}–{fmt}{hi}. Confirm from comps.")
    else:
        st.success(f"✓ {label} {fmt}{val} is within market range {fmt}{lo}–{fmt}{hi}.")
...
with st.sidebar:
    ...
    st.selectbox(
        "Market",
        ["Caribbean Resort", "US Sunbelt Limited Service", "LATAM Mid-Scale"],
        key="market",
    )
    ...
    st.radio(
        "Sponsor Type",
        ["First-time Developer", "Repeat Operator"],
        key="sponsor_type",
        horizontal=False,
    )
...
<div style="padding:1.4rem 0 2.6rem 0;">
  ...
  <div style="font-family:'Manrope',sans-serif;font-size:1.05rem;font-weight:300;line-height:1.65;color:#8a8576;max-width:42ch;">
    <p style="color:#B8B8B8;">
      ...
      evaluate deals under uncertainty — across up to 500,000 simulated futures, including levered and unlevered returns, full operating P&amp;L, capital stack, and sell-vs-hold strategies.
    </p>
  </div>
</div>
""", unsafe_allow_html=True)
...
tab_assumptions, tab_results, tab_mydeals, tab_compare = st.tabs([
    "Assumptions", "Results", "My Deals", "Compare"
])
...
with tab_assumptions:
    ...
    benchmarks = get_benchmarks(st.session_state.get("market", "Caribbean Resort"))
    ...
    sponsor_type = st.session_state.get("sponsor_type", "First-time Developer")

    if sponsor_type == "First-time Developer":
        ...
        <div style="border:1px solid {GOLD};border-radius:7px;padding:1.2rem 1.4rem;margin-top:1rem;">
          ...
          <ul style="font-family:'Manrope',sans-serif;font-size:0.8rem;
                     color:rgba(240,240,240,0.72);line-height:1.7;margin:0;padding-left:1.2rem;">
...
with tab_results:
    if not st.session_state.show_results:
        st.info("Run your analysis from the Assumptions tab to see results here.")
    else:
        ...
        <div style="background:rgba(255,255,255,0.045);
          ...
          <div style="font-family:'Manrope',sans-serif;font-size:0.68rem;letter-spacing:0.14em;
                      text-transform:uppercase;color:rgba(240,240,240,0.48);margin-bottom:0.35rem;">
          ...
          <div style="font-family:'Manrope',sans-serif;font-size:0.9rem;color:rgba(240,240,240,0.72);
        ...
        <div style="margin-top:1.2rem; margin-bottom:0.7rem;">
          ...
          <p style="font-family:'Manrope',sans-serif;font-size:0.84rem;color:rgba(240,240,240,0.55);
        ...
        if len(risk_drivers) == 0:
            ...
            for idx, driver in enumerate(risk_drivers):
                ...
                <div style="background:rgba(255,255,255,0.035);border:1px solid rgba(255,255,255,0.09);
                  ...
                  <div style="font-family:'Manrope',sans-serif;font-size:0.76rem;color:rgba(240,240,240,0.55);line-height:1.55;">
        ...
        <div style="background:linear-gradient(135deg,rgba(201,169,97,0.10),rgba(255,255,255,0.025));
          ...
          <div style="font-family:'Manrope',sans-serif;font-size:0.9rem;color:rgba(240,240,240,0.72);line-height:1.75;">
