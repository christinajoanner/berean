...
with st.sidebar:
    ...
    st.radio(
        "Track Record",
        ["Building", "Growing", "Established", "Platform-Scale"],
        key="sponsor_type",
        horizontal=False,
    )
    ...
    st.markdown(
        f'<div style="font-family:Manrope,sans-serif;font-size:0.72rem;'
        f'color:rgba(240,237,230,0.38);line-height:1.6;">'
        f'Help &amp; Support<br>'
        f'<a href="mailto:hello@thinkberean.com" style="color:{TEAL};text-decoration:none;">'
        f'hello@thinkberean.com</a></div>',
        unsafe_allow_html=True,
    )
...
<div style="padding:1.4rem 0 2.6rem 0;">
  ...
  <div style="font-family:'Cormorant Garamond',serif;font-size:2.6rem;font-weight:400;
              line-height:1.15;letter-spacing:-0.015em;color:#f0ede6;margin-top:2.8rem;margin-bottom:1.5rem;">
    Examine before you <em style="color:#69d3d0;font-style:italic;font-weight:400;">invest</em>.
  </div>
  <div style="font-family:'Manrope',sans-serif;font-size:1.05rem;font-weight:300;
    ...
    Model your deal. Pressure-test assumptions. Build credibility with your LP&mdash;all in one workspace.
  </div>
</div>
""", unsafe_allow_html=True)
...
with tab_assumptions:
    ...
    sponsor_type = st.session_state.get("sponsor_type", "Building")

    if sponsor_type in ("Building", "Growing"):
        ...
        with sc2:
            ...
            inst_backing = st.checkbox("Institutional backing or LP interest", key="institutional_backing")
            prior_raises = st.selectbox(
                "Prior Capital Raises",
                ["None", "1 raise", "2–3 raises"],
                key="prior_raises",
            ) if sponsor_type == "Growing" else None
        ...
        if inst_backing:
            score += 1
            reasons.append("Institutional backing or LP interest")
        if sponsor_type == "Growing" and prior_raises and prior_raises != "None":
            score += 1
            reasons.append(f"Prior fundraising history: {prior_raises}")

        max_score = 9 if sponsor_type == "Growing" else 8
        ...
        <div style="border:1px solid {GOLD};border-radius:7px;padding:1.2rem 1.4rem;margin-top:1rem;">
          <div style="font-family:'Manrope',sans-serif;font-size:0.65rem;letter-spacing:0.14em;
            ...
            {sponsor_type} Track Record Profile
          </div>
    ...
    else:  # Established or Platform-Scale
        sc1, sc2 = st.columns(2)
        with sc1:
        ...
        with sc2:
            ...
            repeat_lps = st.checkbox("Repeat LP relationships", key="repeat_lps")
        ...
        if repeat_lps:
            score += 1
            reasons.append("Repeat LP relationships")
        ...
        max_score = 8 if sponsor_type == "Established" else 9
        score_color = GOLD if score >= 5 else (WHITE if score >= 3 else RED)
...
with tab_results:
    if not st.session_state.get("show_results"):
        ...
        fig.add_trace(go.Histogram(
            x=primary, nbinsx=80,
            marker_color="rgba(201,169,97,0.68)",
            marker_line_color="rgba(201,169,97,0.30)", marker_line_width=0.4,
            opacity=1.0,
            hovertemplate=f"{primary_unit}: %{{x:.1f}}<br>Count: %{{y:,}}<extra></extra>",
        ))
        for val, label, pos in [
            (p10, "P10", "top"), (p50, "P50", "top"), (p90, "P90", "top"),
        ]:
            fig.add_vline(x=val, line_dash="dash", line_color=TEAL, line_width=1.8,
                          annotation_text=f"<b>{label}</b> {val:.1f}%",
                          annotation_font_color=TEAL, annotation_font_family="Manrope",
                          annotation_font_size=12, annotation_position=pos)
        fig.add_vline(x=target_value, line_dash="dot", line_color=WHITE, line_width=1.2,
                      annotation_text=f"<b>{target_label}</b>",
                      annotation_font_color=WHITE, annotation_font_family="Manrope",
        ...
        _rec_border = TEAL if verdict == "Proceed" else (GOLD if verdict == "Borderline" else RED)
        st.markdown(f"""
        <div style="background:rgba(255,255,255,0.03);
                    border:1px solid rgba(255,255,255,0.08);
                    border-top:3px solid {_rec_border};
                    border-radius:7px;
                    padding:1.4rem 1.6rem;
                    margin-top:0.8rem;
                    margin-bottom:1.1rem;">
          <div style="font-family:'Manrope',sans-serif;font-size:0.65rem;letter-spacing:0.18em;
                      text-transform:uppercase;color:{_rec_border};margin-bottom:0.6rem;font-weight:600;">
            Recommendation
          </div>
          <div style="font-family:'Cormorant Garamond',serif;font-size:2.2rem;color:{_rec_border};
                      line-height:1.1;margin-bottom:0.55rem;font-weight:400;">
            {verdict}
          </div>
          <div style="font-family:'Manrope',sans-serif;font-size:0.92rem;color:rgba(240,237,230,0.72);
...
with tab_memo:
    if not st.session_state.get("show_results"):
        ...
        if _v is not None and _pt is not None:
            _memo_border = TEAL if _v == "Proceed" else (GOLD if _v == "Borderline" else RED)
            st.markdown(f"""
            <div style="background:rgba(255,255,255,0.03);
                        border:1px solid rgba(255,255,255,0.08);
                        border-top:3px solid {_memo_border};
                        border-radius:7px;
                        padding:1.4rem 1.6rem;margin-bottom:1.2rem;">
              <div style="font-family:'Manrope',sans-serif;font-size:0.65rem;letter-spacing:0.18em;
                          text-transform:uppercase;color:{_memo_border};margin-bottom:0.6rem;font-weight:600;">
                Recommendation
              </div>
              <div style="font-family:'Cormorant Garamond',serif;font-size:2.2rem;color:{_memo_border};
                          line-height:1.1;margin-bottom:0.55rem;font-weight:400;">{_v}</div>
              <div style="font-family:'Manrope',sans-serif;font-size:0.9rem;color:rgba(240,237,230,0.72);
                          line-height:1.7;max-width:80ch;">{_vt}</div>
              <div style="font-family:'Manrope',sans-serif;font-size:0.82rem;color:rgba(240,237,230,0.55);
                          margin-top:0.9rem;border-top:1px solid rgba(255,255,255,0.07);padding-top:0.7rem;">
