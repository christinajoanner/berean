...
def save_to_airtable(table, payload):
    """Save a row to Airtable only after Airtable credentials are configured."""
...
if st.session_state.show_results:
    ...
    with st.form("feedback_form"):
        ...
        if submitted:
            ...
            if saved_feedback:
                ...
                st.success("Feedback captured locally. Airtable is not configured yet, so it was not saved externally.")
    ...
    with st.form("save_analysis_form"):
        ...
        if save_analysis_clicked:
            ...
            saved_deal = save_to_airtable("Deals", deal_payload)
            saved_assumptions = save_to_airtable("Assumptions_Metadata", assumption_payload)

            if saved_deal and saved_assumptions:
                st.session_state.analysis_saved = True
                st.success("Analysis saved.")
            elif AIRTABLE_BASE_ID == "YOUR_BASE_ID" or AIRTABLE_API_KEY == "YOUR_API_KEY":
                st.info("Analysis is ready to save, but Airtable is not configured yet. Add your Airtable Base ID and API token to enable tracking.")
            else:
                st.error("Could not save analysis to Airtable. Check your table names, field names, Base ID, and API token.")
