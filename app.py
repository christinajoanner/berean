...
# ══════════════════════════════════════════════════════════════════════════════
# DECISION INTELLIGENCE HELPERS — v2.2
# ══════════════════════════════════════════════════════════════════════════════

def decision_verdict(prob_target, p10, p50, dscr_clean, has_debt, target_value):
    """Create a plain-English investment verdict from the simulated distribution."""
    ...
    if prob_target >= 70 and p10 >= target_value * 0.55 and not dscr_risk:
        return (
            "Proceed",
            GREEN,
            "This deal shows a strong probability of meeting the target return, with acceptable downside protection based on the simulated range of outcomes."
        )
    elif prob_target >= 45:
        return (
            "Borderline",
            GOLD,
            "This deal may work, but the outcome is sensitive to several assumptions. The next step is to validate the main risk drivers before moving forward."
        )
    else:
        return (
            "High Risk",
            RED,
            "This deal does not clear the target return often enough across simulated outcomes. The current assumptions may need to be revised before committing capital."
        )


def estimate_risk_drivers(p):
    """Simple v2.2 risk-driver ranking based on the relative width of user input ranges."""
    drivers = []
    drivers.append(("Exit Cap Rate", (p['cap_hi'] - p['cap_lo']) / max(p['cap_mode'], 0.01), "Exit valuation risk"))
    drivers.append(("ADR", (p['adr_hi'] - p['adr_lo']) / max(p['adr_mode'], 1), "Revenue pricing risk"))
    drivers.append(("Cost per Key", (p['cpk_hi'] - p['cpk_lo']) / max(p['cpk_mode'], 1), "Development cost risk"))
    drivers.append(("Occupancy", (p['occ_hi'] - p['occ_lo']) / max(p['occ_mode'], 1), "Demand / absorption risk"))
    drivers.append(("ADR Growth", (p['adr_g_hi'] - p['adr_g_lo']) / max(abs(p['adr_g_mode']), 1), "Future growth risk"))
    return sorted(drivers, key=lambda x: x[1], reverse=True)[:3]


def what_needs_to_be_true(p, prob_target, target_probability=75):
    """Plain-English suggestions for improving the probability of clearing the target."""
    ...
    return [
        ...
        "The next step is to validate the main assumptions with market data and lender feedback.",
        "Focus diligence on the top downside drivers before committing capital.",
    ]
...
if st.session_state.show_results:
    ...
    # ── Risk Drivers ──────────────────────────────────────────────────────────
    risk_drivers = estimate_risk_drivers(p)
    ...
    <div style="margin-top:1.2rem; margin-bottom:0.7rem;">
      ...
      <p style="font-family:'Manrope',sans-serif;
                ...
                line-height:1.6;
                margin-top:-0.35rem;">
        These are the assumptions most likely to move the return distribution based on the width of the input ranges.
      </p>
    </div>
    """, unsafe_allow_html=True)

    rd_cols = st.columns(3, gap="small")
    for idx, (driver, score, reason) in enumerate(risk_drivers):
