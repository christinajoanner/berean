...
<div style="padding:1.4rem 0 2.6rem 0;">
  ...
  <div style="font-family:'Manrope',sans-serif;font-size:1.05rem;font-weight:300;line-height:1.65;color:#8a8576;max-width:48ch;">
    ...
    Evaluate deals under uncertainty. Up to 1,000,000 simulated futures,
...
def true_sensitivity_analysis(res, primary_metric):
    ...
    for name, values in inputs.items():
        ...
        if np.isfinite(corr):
            drivers.append((name, corr, abs(corr)))

    drivers = sorted(drivers, key=lambda x: x[2], reverse=True)
    return drivers[:3]
